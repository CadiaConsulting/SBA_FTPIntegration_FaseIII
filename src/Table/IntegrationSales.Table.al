table 50010 "IntegrationSales"
{
    Caption = 'Integration Sales';
    DataClassification = CustomerContent;
    DrillDownPageId = IntegrationSales;
    ...
    fields
    {
        ...
        field(60; "Tax (PIS) Order"; Decimal)
        {
            ...
            CalcFormula = sum(IntegrationSales."Tax (PIS) Line" where("No." = field("No.")));
        }
        field(61; "Tax (COFINS) Order"; Decimal)
        {
            ...
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = sum(IntegrationSales."Tax (COFINS) Line" where("No." = field("No.")));
        }
        field(100; "Error Order"; Integer)
        {
            Caption = 'Error Order';
            FieldClass = FlowField;
            CalcFormula = count(IntegrationSales where("No." = field("No."),
                                                            "Status" = filter(2)));
        }
        field(110; "Errors Import Excel"; Integer)
        {
            ...
            Editable = false;
            FieldClass = FlowField;
            CalcFormula = count(IntegrationErros where("Integration Type" = filter(0),
